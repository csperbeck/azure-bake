...
import { DeploymentContext, Logger, BakeVariable } from "@azbake/core"
...
export class BakeRunner {
    ...
    private async _executeBakeLoop(ingredientNames: string[], finished: string[], ctx: DeploymentContext, resc_types: string[]): Promise<boolean> {
        ...
        for (let i = 0; i < count; ++i) {
            ...
            if (resc_types.indexOf(ingredient.properties.type.toLowerCase()) > -1) {
    ...
    private async _bakeRegion(ctx: DeploymentContext): Promise<boolean> {
        ...
        try {
            ...
            let allowedTypes = (ctx.Environment.variables.get('types') || <BakeVariable>{} ).value(ctx).split(',')
            ctx.Logger.log('Allowed Types: ' + allowedTypes)
            ...
            while (loopHasRemaining) {
                try {
                    loopHasRemaining = await this._executeBakeLoop(ingredientNames, finished, ctx, allowedTypes)
                }
                catch{
                    throw new Error()
                }
